...
func (_q *SubmissionQuery) QueryTemplate() *TemplateQuery {
...
// QuerySubmitters chains the current query on the "submitters" edge.
func (_q *SubmissionQuery) QuerySubmitters() *SubmitterQuery {
	query := (&SubmitterClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SubmissionQuery) QueryEvents() *EventQuery {
...
func (_q *SubmissionQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *SubmissionQuery) FirstIDX(ctx context.Context) string {
...
func (_q *SubmissionQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *SubmissionQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *SubmissionQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *SubmissionQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *SubmissionQuery) Clone() *SubmissionQuery {
	...
	return &SubmissionQuery{
		...
		withSubmitters: _q.withSubmitters.Clone(),
...
func (_q *SubmissionQuery) WithTemplate(opts ...func(*TemplateQuery)) *SubmissionQuery {
	query := (&TemplateClient{config: _q.config}).Query()
...
// WithSubmitters tells the query-builder to eager-load the nodes that are connected to
// the "submitters" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SubmissionQuery) WithSubmitters(opts ...func(*SubmitterQuery)) *SubmissionQuery {
	query := (&SubmitterClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSubmitters = query
	return _q
}

// WithEvents tells the query-builder to eager-load the nodes that are connected to
// the "events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *SubmissionQuery) WithEvents(opts ...func(*EventQuery)) *SubmissionQuery {
	query := (&EventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(submission.FieldCreateBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Submission.Query().
//		Select(submission.FieldCreateBy).
//		Scan(ctx, &v)
...
func (_q *SubmissionQuery) prepareQuery(ctx context.Context) error {
	...
	if submission.Policy == nil {
		return errors.New("ent: uninitialized submission.Policy (forgotten import ent/runtime?)")
	}
	if err := submission.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *SubmissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Submission, error) {
	...
	if query := _q.withTemplate; query != nil {
		if err := _q.loadTemplate(ctx, query, nodes, nil,
			func(n *Submission, e *Template) { n.Edges.Template = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubmitters; query != nil {
		if err := _q.loadSubmitters(ctx, query, nodes,
			func(n *Submission) { n.Edges.Submitters = []*Submitter{} },
			func(n *Submission, e *Submitter) { n.Edges.Submitters = append(n.Edges.Submitters, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEvents; query != nil {
		if err := _q.loadEvents(ctx, query, nodes,
			func(n *Submission) { n.Edges.Events = []*Event{} },
			func(n *Submission, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubmissionQuery) loadTemplate(ctx context.Context, query *TemplateQuery, nodes []*Submission, init func(*Submission), assign func(*Submission, *Template)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Submission)
	for i := range nodes {
...
func (_q *SubmissionQuery) loadSubmitters(ctx context.Context, query *SubmitterQuery, nodes []*Submission, init func(*Submission), assign func(*Submission, *Submitter)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(submitter.FieldSubmissionID)
	}
	query.Where(predicate.Submitter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(submission.SubmittersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SubmissionQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Submission, init func(*Submission), assign func(*Submission, *Event)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Submission)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(event.FieldSubmissionID)
	}
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(submission.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SubmissionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(submission.Table, submission.Columns, sqlgraph.NewFieldSpec(submission.FieldID, field.TypeString))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTemplate != nil {
			_spec.Node.AddColumnOnce(submission.FieldTemplateID)
		}
	}
