...
type TemplateQuery struct {
	...
	withFolder      *TemplateFolderQuery
...
func (_q *TemplateQuery) QueryFolder() *TemplateFolderQuery {
...
func (_q *TemplateQuery) QuerySubmissions() *SubmissionQuery {
...
func (_q *TemplateQuery) WithFolder(opts ...func(*TemplateFolderQuery)) *TemplateQuery {
	query := (&TemplateFolderClient{config: _q.config}).Query()
...
func (_q *TemplateQuery) WithSubmissions(opts ...func(*SubmissionQuery)) *TemplateQuery {
	query := (&SubmissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TemplateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Template, error) {
	...
	if query := _q.withFolder; query != nil {
		if err := _q.loadFolder(ctx, query, nodes, nil,
			func(n *Template, e *TemplateFolder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubmissions; query != nil {
		if err := _q.loadSubmissions(ctx, query, nodes,
			func(n *Template) { n.Edges.Submissions = []*Submission{} },
			func(n *Template, e *Submission) { n.Edges.Submissions = append(n.Edges.Submissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TemplateQuery) loadFolder(ctx context.Context, query *TemplateFolderQuery, nodes []*Template, init func(*Template), assign func(*Template, *TemplateFolder)) error {
...
func (_q *TemplateQuery) loadSubmissions(ctx context.Context, query *SubmissionQuery, nodes []*Template, init func(*Template), assign func(*Template, *Submission)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(submission.FieldTemplateID)
	}
	query.Where(predicate.Submission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(template.SubmissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *TemplateQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withFolder != nil {
			_spec.Node.AddColumnOnce(template.FieldFolderID)
		}
	}
