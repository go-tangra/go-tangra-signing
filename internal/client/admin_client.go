...
func NewAdminClient(ctx *bootstrap.Context, certManager *cert.CertManager) (*AdminClient, func(), error) {
	l := ctx.NewLoggerHelper("signing/client/admin")
...
func loadAdminClientTLS(_ *cert.CertManager, l *log.Helper) (credentials.TransportCredentials, error) {
	...
	// Use the signing client cert if available, otherwise use server cert
	clientCertPath := filepath.Join(certsDir, "signing", "signing.crt")
	clientKeyPath := filepath.Join(certsDir, "signing", "signing.key")

	// Fall back to server cert if client cert not available
	if _, err := os.Stat(clientCertPath); os.IsNotExist(err) {
		clientCertPath = filepath.Join(certsDir, "signing-server", "server.crt")
		clientKeyPath = filepath.Join(certsDir, "signing-server", "server.key")
	}
